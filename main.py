...
def extract_zones(page):
    ...
    # Semplifica la griglia: raggruppa linee vicine (entro 10px)
    def simplify(lines, threshold=30):
        simplified = []
        for l in sorted(lines):
            if not simplified or l - simplified[-1] > threshold:
                simplified.append(l)
        return simplified
    ...
    # Trova testi (label) con coordinate
    skip = ['email','www','cliente','arch','prospetto','data','binovamilano','mirko','buonocore','@','http']
    text_items = []
    for b in page.get_text("dict")["blocks"]:
        ...
        for line in b["lines"]:
            for span in line["spans"]:
                ...
                if not text or not any(c.isalpha() for c in text) or len(text) < 3: continue
                if any(kw in text.lower() for kw in skip): continue
                cx = (span["bbox"][0] + span["bbox"][2]) / 2
                cy = (span["bbox"][1] + span["bbox"][3]) / 2
                text_items.append({"text": text, "cx": cx, "cy": cy})

    # Associa ogni testo alla cella della griglia semplificata
    cells = {}
    for item in text_items:
        col = next(((sx[i], sx[i+1]) for i in range(len(sx)-1) if sx[i] <= item["cx"] <= sx[i+1]), None)
        row = next(((sy[i], sy[i+1]) for i in range(len(sy)-1) if sy[i] <= item["cy"] <= sy[i+1]), None)
        if col and row:
            key = (col[0], row[0], col[1], row[1])
            cells.setdefault(key, []).append(item["text"])

    # Costruisce le zone finali
    zones = []
    seen = {}
...
async def colorize(req: ColorizeRequest):
    try:
        ...
        for zone in req.zones:
            if not zone.color: continue
            h = zone.color.lstrip('#')
            r, g, b = int(h[0:2],16)/255, int(h[2:4],16)/255, int(h[4:6],16)/255
            page.draw_rect(
                fitz.Rect(zone.x0, zone.y0, zone.x1, zone.y1),
            ...
            applied += 1
        return {
            "pdf_base64": base64.b64encode(doc.tobytes()).decode(),
